...
def susceptability_query(country):
    ...
    if (counter >= 7):
        return("According to the ReliefWeb database for historical natural disaster data, " 
            + country + 
            " is susceptible to natural disasters, due to the increased amount of natural disasters in its history.\n")
    else:
        return("According to the ReliefWeb database for historical natural disaster data, " + country + 
